...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex items-center justify-between no-print">
        <div>
            <a href="{{ route('pasien.rekam-medis.index') }}" class="text-xs font-bold text-blue-600 hover:underline flex items-center gap-1">
                ⬅️ Kembali ke Daftar
            </a>
            <h3 class="text-2xl font-bold text-slate-800 mt-2">Detail Rekam Medis Anda</h3>
            <p class="text-slate-500 text-sm mt-1">Laporan rekam medis lengkap pemeriksaan kesehatan Anda.</p>
        </div>
        <div>
            <button onclick="window.print()" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition flex items-center gap-2 shadow-md shadow-blue-200">
                🖨️ Cetak Dokumen
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-100/50 p-10 relative overflow-hidden" id="printable-area">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8 pb-6 border-b border-slate-100">
            <div class="space-y-4">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Data Pasien</h4>
                <div class="space-y-1">
                    <p class="text-base font-extrabold text-slate-800">{{ auth()->user()->nama_lengkap }}</p>
                    <p class="text-xs text-slate-500">Telepon: {{ auth()->user()->no_telepon ?? '-' }}</p>
                    <p class="text-xs text-slate-500">Alamat: {{ auth()->user()->alamat ?? '-' }}</p>
                </div>
            </div>
            <div class="space-y-4 text-left md:text-right">
                <h4 class="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none">Dokter Pemeriksa</h4>
                <div class="space-y-1">
                    ...
                    <p class="text-xs text-slate-500">Tanggal Periksa: <span class="font-semibold text-slate-800">{{ $rekamMedis->tanggal_periksa->format('d F Y') }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-8">
            <!-- Keluhan -->
            <div class="space-y-2">
                <h5 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Keluhan Utama</h5>
            ...
            <!-- Diagnosa -->
            <div class="space-y-2">
                <h5 class="text-xs font-bold text-slate-400 uppercase tracking-widest">Diagnosa</h5>
        ...
        <div class="mt-16 pt-8 border-t border-slate-100 flex justify-between items-end">
            <div class="text-xs text-slate-400">
                <p>Klinik Pratama Orinda</p>
                <p class="mt-1">Dokumen dicetak secara mandiri oleh Pasien</p>
            </div>
