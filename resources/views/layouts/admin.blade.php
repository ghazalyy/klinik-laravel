...
<head>
    ...
    <title>Admin | @yield('title', 'Dashboard') — Klinik Orinda</title>
    ...
    <div class="flex h-screen">
        ...
        <aside class="w-64 sidebar-glass text-white hidden lg:flex flex-col flex-shrink-0 z-40">
            <div class="p-6 border-b border-white/5">
                ...
                <p class="text-[10px] uppercase tracking-[0.2em] text-blue-400 mt-2 font-bold opacity-70">Administrator</p>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                @php
                    ...
                    $links = [
                        ['label' => 'Dashboard', 'icon' => '📊', 'route' => 'admin.dashboard', 'patterns' => ['admin.dashboard']],
                        ['label' => 'CRM Analitik', 'icon' => '📈', 'route' => 'admin.crm.dashboard', 'patterns' => ['admin.crm.*']],
                        ['label' => 'SPK Dokter', 'icon' => '🏆', 'route' => 'admin.spk', 'patterns' => ['admin.spk']],
                    ];
                    $managements = [
                        ['label' => 'Kelola Dokter', 'icon' => '👨‍⚕️', 'route' => 'admin.dokter.index', 'patterns' => ['admin.dokter.*']],
                        ['label' => 'Kelola Jadwal', 'icon' => '📅', 'route' => 'admin.jadwal.index', 'patterns' => ['admin.jadwal.*']],
                        ['label' => 'Kelola Antrean', 'icon' => '🎟️', 'route' => 'admin.antrean.index', 'patterns' => ['admin.antrean.*']],
                        ['label' => 'Kelola Pengguna', 'icon' => '👥', 'route' => 'admin.pengguna.index', 'patterns' => ['admin.pengguna.*']],
                    ];
                    $finance = [
                        ['label' => 'Verifikasi Bayar', 'icon' => '💳', 'route' => 'admin.pembayaran.verifikasi', 'patterns' => ['admin.pembayaran.verifikasi']],
                        ['label' => 'Laporan Rekap', 'icon' => '📋', 'route' => 'admin.laporan', 'patterns' => ['admin.laporan']],
                    ];
                @endphp

                <p class="text-[10px] text-blue-400 font-bold uppercase px-4 py-3 mt-2 opacity-50 tracking-widest">Main Menu</p>
                @foreach($links as $link)
                    @php $active = false; foreach($link['patterns'] as $p) if(Route::is($p)) $active = true; @endphp
                    <a href="{{ route($link['route']) }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ $active ? 'nav-active' : '' }}">
                        <span class="text-lg opacity-70">{{ $link['icon'] }}</span>
                        <span class="font-medium text-[13px]">{{ $link['label'] }}</span>
                    </a>
                @endforeach

                <p class="text-[10px] text-blue-400 font-bold uppercase px-4 py-3 mt-6 opacity-50 tracking-widest">Management</p>
                @foreach($managements as $link)
                    @php $active = false; foreach($link['patterns'] as $p) if(Route::is($p)) $active = true; @endphp
                    <a href="{{ route($link['route']) }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ $active ? 'nav-active' : '' }}">
                        <span class="text-lg opacity-70">{{ $link['icon'] }}</span>
                        <span class="font-medium text-[13px]">{{ $link['label'] }}</span>
                ...
                <p class="text-[10px] text-blue-400 font-bold uppercase px-4 py-3 mt-6 opacity-50 tracking-widest">Finance</p>
            ...
            <div class="p-4 bg-black/20 mt-auto">
                <div class="flex items-center gap-3 mb-4 px-2">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-[10px] text-blue-300 opacity-70">Root Admin</p>
                    </div>
                </div>
