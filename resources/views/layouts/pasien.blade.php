    ...
    <div class="flex h-screen">
        ...
        <aside class="w-64 sidebar-glass text-white hidden lg:flex flex-col flex-shrink-0 z-40">
            ...
            <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
                ...
                <a href="{{ route('pasien.dashboard') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ request()->routeIs('pasien.dashboard') ? 'nav-active' : '' }}">
                    <span class="text-lg opacity-70">🏠</span>
                    <span class="font-medium text-[13px]">Dashboard</span>
                </a>

                <a href="{{ route('pasien.booking.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ request()->routeIs('pasien.booking.index') ? 'nav-active' : '' }}">
                    <span class="text-lg opacity-70">📅</span>
                    <span class="font-medium text-[13px]">Booking Konsultasi</span>
                </a>

                <a href="{{ route('pasien.booking.riwayat') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ request()->routeIs('pasien.booking.riwayat') ? 'nav-active' : '' }}">
                    <span class="text-lg opacity-70">📋</span>
                ...
                <a href="{{ route('pasien.antrean.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ request()->routeIs('pasien.antrean.index') ? 'nav-active' : '' }}">
                    <span class="text-lg opacity-70">🎫</span>
                    <span class="font-medium text-[13px]">Antrean Offline</span>
                </a>

                <a href="{{ route('pasien.antrean.riwayat') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl transition text-sm text-blue-100 hover:bg-white/5 {{ request()->routeIs('pasien.antrean.riwayat') ? 'nav-active' : '' }}">
                    <span class="text-lg opacity-70">📃</span>
            ...
            <div class="p-4 bg-black/20 mt-auto">
                <div class="flex items-center gap-3 mb-4 px-2">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-[10px] text-blue-300 opacity-70">Pasien Terdaftar</p>
                    </div>
                </div>
        ...
        <main class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <div class="flex-1 overflow-y-auto p-8">
                ...
                @if(session('info'))
                    <div class="mb-8 p-4 bg-blue-50 border border-blue-100 text-blue-700 rounded-2xl flex items-center gap-3 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500">
                        <div class="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center text-white text-xs">ℹ</div>
                        <p class="text-[13px] font-semibold">{{ session('info') }}</p>
                    </div>
                @endif
